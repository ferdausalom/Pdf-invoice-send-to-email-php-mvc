...
file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/invoices/invoice.pdf', $output);
...
try {
	...
	$phpmailer->addAddress($address->email, $address->username);     //Add a recipient
	$phpmailer->addReplyTo('info@example.com', 'Information');

	//Attachments
	$phpmailer->addAttachment($_SERVER['DOCUMENT_ROOT'] . '/invoices/invoice.pdf');         //Add attachments
	...
	unlink($_SERVER['DOCUMENT_ROOT'] . '/invoices/invoice.pdf');

	return redirect("emailed");
} catch (Exception $e) {
	return redirect("404");
